...
import {
    ...
    Info, X, ArrowRight, Clock, AlertCircle, Briefcase
} from "lucide-react"
...
export default function JobRequestManagementPage() {
    ...
    const [userDept, setUserDept] = React.useState("") // Added for department check
    ...
    React.useEffect(() => {
        ...
        // Fetch profile to get accurate department for engiconnect
        if (storedId) {
            fetch(`/api/user?id=${encodeURIComponent(storedId)}`)
                ...
                .catch(() => setUserDept("sales")); // Default fallback
        }
    ...
    // HELPER: Improved Check for Sales Access
    const canCreateJob = 
        userRole?.toUpperCase() === "SALES" || 
        userDept === "sales" || 
        userDept === ""; // Empty check allows button to show while loading

    const handleCreateNew = () => {
        if (!canCreateJob) return; 
        router.push('/request/job/add')
    }
                ...
                <SidebarInset className="relative bg-[#F4F7F7] pb-24 md:pb-0 font-sans">
                    ...
                    <main className="p-4 md:p-8 max-w-7xl mx-auto w-full space-y-6">
                                ...
                                <div className="bg-white p-8 rounded-[24px] shadow-2xl border border-zinc-200 w-[320px] pointer-events-auto animate-in zoom-in-95 duration-200">
                                    ...
                                    <div className="space-y-2 mb-8">
                                        <h3 className="font-black text-xl tracking-tight text-zinc-900 uppercase">
                                            {guideStep === 1 && "Monitor Jobs"}
                                            {guideStep === 2 && "Quick Toggles"}
                                            {guideStep === 3 && "Filtering"}
                                        </h3>
                                        <p className="text-sm leading-relaxed text-zinc-500">
                                            {guideStep === 1 && "Track all engineering job projects in real-time."}
                        ...
                        <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            <StatCard label="ALL JOBS" val={requests.length} icon={Briefcase} isActive={filterStatus === null} onClick={() => setFilterStatus(null)} />
                            ...
                            <StatCard label="COMPLETION" val="94%" icon={TrendingUp} isStatic />
                            <StatCard label="UPTIME" val="100%" icon={ShieldCheck} isStatic />
                        </section>
                                    ...
                                    paginatedItems.map((r) => (
                                        <div key={r.id} className="grid grid-cols-1 md:grid-cols-6 gap-4 p-6 items-center hover:bg-zinc-50/40 transition-colors">
                                            ...
                                            <div className="col-span-2 flex flex-col">
                                                ...
                                                <span className="text-[9px] text-zinc-400 font-bold uppercase">{r.clientName || "General Client"}</span>
                                            </div>
                                            ...
                                            <div className="flex justify-end">
                                                <Button 
                                                    variant="ghost" 
                                                    onClick={() => router.push(`/request/job/${r.id}`)}
                    ...
                    {/* UPDATED MOBILE FAB RESTRICTION */}
                    {canCreateJob && (
