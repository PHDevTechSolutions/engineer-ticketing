        ...
        <ProtectedPageWrapper>
            <SidebarProvider defaultOpen={false}>
                ...
                <SidebarInset className="bg-[#F2F4F7] relative min-h-screen font-sans pb-safe">
                    ...
                    <header className="hidden md:flex h-20 items-center justify-between px-8 bg-white border-b border-gray-100 sticky top-0 z-50">
                                    ...
                                    <div className="size-10 rounded-xl overflow-hidden bg-gray-900">
                                        {userDetails.profilePicture ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-white font-bold text-xs uppercase">
                                                {userDetails.Firstname ? userDetails.Firstname.substring(0, 2) : "EN"}
                                            </div>
                                        )}
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1">{getGreeting()}</p>
                                        <h2 className="text-xl font-bold text-gray-900 tracking-tight leading-none">Welcome, {userDetails.Firstname || "User"}</h2>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="flex items-center gap-4">
                            ...
                            <div className="relative" ref={notifRef}>
                                <button onClick={() => setShowNotifDropdown(!showNotifDropdown)} className={cn("p-2.5 rounded-xl transition-all relative", showNotifDropdown ? "bg-red-50 text-[#E33636]" : "text-gray-400 hover:text-[#E33636] hover:bg-red-50")}>
                                    <Bell size={20} />
                                ...
                                {showNotifDropdown && (
                                    <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 py-4 z-[60] animate-in fade-in zoom-in duration-200">
                                        <div className="px-5 pb-3 border-b border-gray-50 flex justify-between items-center">
                                            <h3 className="font-bold text-gray-900 text-sm">Notifications</h3>
                                            <span className="text-[10px] bg-red-100 text-[#E33636] px-2 py-0.5 rounded-full font-black uppercase">{totalNotifications} Alerts</span>
                                        </div>
                                        <div className="max-h-[380px] overflow-y-auto py-2">
                                            {totalNotifications === 0 ? <div className="py-10 text-center"><p className="text-[10px] text-gray-400 font-bold uppercase italic">No pending tasks</p></div> : (
                                                <>
                            ...
                            <div className="flex justify-between items-start mb-6">
                                <div className="flex items-center gap-3">
                                    <div className="size-11 rounded-full overflow-hidden border-2 border-white/30 bg-white/10">
                                        {userDetails.profilePicture ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-white font-bold">{userDetails.Firstname ? userDetails.Firstname[0] : "E"}</div>
                                        )}
                                    </div>
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => router.push('/messages')} className="p-2.5 bg-white/10 rounded-full border border-white/10 text-white relative">
                                        ...
                                        {notifications.unreadMessages > 0 && <span className="absolute -top-1 -right-1 flex min-w-[16px] h-4 items-center justify-center rounded-full bg-blue-400 text-[9px] font-bold text-white border-2 border-[#E33636] px-0.5 shadow-sm">{notifications.unreadMessages}</span>}
                                    </button>
                    ...
                    <main className="px-4 -mt-8 space-y-8 pb-32 relative z-20 md:mt-0 md:px-12 md:py-10 animate-in fade-in duration-700">
                        {/* Service Grid */}
                        <section>
                                ...
                                {isDataLoading ? (
                                    <div className="grid grid-cols-3 md:grid-cols-6 gap-6">
                                        {[...Array(6)].map((_, i) => (
                                            <div key={i} className="flex flex-col items-center space-y-3">
                                                <Skeleton className="size-14 md:size-16 rounded-2xl bg-gray-100 animate-pulse" />
                                    ...
                                    <>
                                        <div className="md:hidden">
                                            <div className="flex overflow-x-auto snap-x snap-mandatory no-scrollbar py-3" onScroll={handleScroll} ref={scrollRef}>
                                                    ...
                                                    <div key={pageIndex} className="min-w-full grid grid-cols-3 gap-y-5 px-2 snap-center">
                                                        {services.slice(pageIndex * itemsPerPage, (pageIndex + 1) * itemsPerPage).map((service, i) => (
                                                            <button key={i} onClick={() => router.push(service.path)} className="flex flex-col items-center transition-all active:scale-90">
                                                                <div className="size-16 bg-gray-100 rounded-2xl flex items-center justify-center mb-3 relative">
                                        ...
                                        <div className="hidden md:grid grid-cols-6 gap-6">
                                            {services.map((service, i) => (
                                                <button key={i} onClick={() => router.push(service.path)} className="flex flex-col items-center group transition-transform hover:-translate-y-1">
                        ...
                        <section className="grid grid-cols-3 gap-3">
                            <StatCard label="Pending" value={isDataLoading ? "--" : totalNotifications} icon={Layers} color="text-[#E33636]" loading={isDataLoading} />
                            <StatCard label="Messages" value={isDataLoading ? "--" : notifications.unreadMessages} icon={MessageSquare} color="text-blue-600" loading={isDataLoading} />
                            <StatCard label="Success" value={isDataLoading ? "--" : notifications.dialuxCompleted} icon={CheckCircle2} color="text-green-600" loading={isDataLoading} />
                        </section>
                        ...
                        <section className="space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={() => router.push('/notifications')} className="text-[10px] font-bold text-[#E33636] uppercase tracking-wider flex items-center gap-1">View All <ArrowUpRight size={12} /></button>
                            </div>
                            <div className="space-y-3">
                                {isDataLoading ? [...Array(3)].map((_, i) => (
                                    <div key={i} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center gap-4">
                                        <Skeleton className="size-10 rounded-xl bg-gray-50" />
                                        <div className="space-y-2 flex-1"><Skeleton className="h-3 w-1/2 bg-gray-50" /><Skeleton className="h-2 w-1/4 bg-gray-50" /></div>
                                    </div>
                                )) : recentActivity.length > 0 ? recentActivity.map((item) => (
                                    <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between group active:scale-[0.98] transition-all cursor-pointer" onClick={() => {
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className={cn("size-10 rounded-xl flex items-center justify-center", item.type === 'DIAlux' ? "bg-indigo-50 text-indigo-600" : item.type === 'Job' ? "bg-orange-50 text-orange-600" : "bg-emerald-50 text-emerald-600")}>
                                                {item.type === 'DIAlux' ? <Monitor size={18} /> : item.type === 'Job' ? <FileText size={18} /> : <StreetLightIcon size={18} />}
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-2 mb-0.5"><span className="text-[8px] font-black px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded uppercase tracking-widest">{item.type}</span><p className="text-xs font-bold text-gray-900 line-clamp-1">{item.projectTitle || item.requestTitle || "New Activity"}</p></div>
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                                {activeTab === "Monitoring" ? (
                                    <>
                                        <ActivityCard label="In-Testing" value={notifications.testingActive} icon={ClipboardCheck} loading={isDataLoading} />
                                        <ActivityCard label="Critical" value={notifications.testingOverdue} icon={AlertTriangle} isAlert={notifications.testingOverdue > 0} loading={isDataLoading} />
                                        <ActivityCard label="Site Visits" value={notifications.siteVisit} icon={CalendarCheck} loading={isDataLoading} />
                                        <ActivityCard label="Shop Review" value={notifications.shopDrawing} icon={StreetLightIcon as LucideIcon} loading={isDataLoading} />
                                    </>
                                ) : (
                                    <div className="col-span-2 md:col-span-4 py-10 flex flex-col items-center justify-center bg-white rounded-2xl border border-dashed border-gray-200"><p className="text-[10px] font-bold text-gray-400 uppercase">{activeTab} Details Syncing...</p></div>
                                )}
                            </div>
                        </section>
                    </main>

                    <button onClick={() => router.push('/request/testing/add')} className="fixed bottom-8 right-6 size-14 bg-[#E33636] text-white rounded-full shadow-2xl flex items-center justify-center active:scale-90 transition-all z-50"><Plus size={24} strokeWidth={3} /></button>
                </SidebarInset>
            </SidebarProvider>
...
function StatCard({ label, value, icon: Icon, color, loading }: { label: string, value: any, icon: any, color: string, loading: boolean }) {
    return (
        <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center transition-all">
            <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter mb-1">{label}</span>
            <div className="flex items-baseline gap-1">
                {loading ? <Skeleton className="h-5 w-8 bg-gray-50" /> : <span className={cn("text-xl font-black", color)}>{value}</span>}
...
function ActivityCard({ label, value, icon: Icon, isAlert, loading }: { label: string, value: number, icon?: any, isAlert?: boolean, loading: boolean }) {
    return (
        <div className={cn("bg-white p-4 rounded-2xl shadow-sm border flex flex-col gap-1 min-h-[100px] relative overflow-hidden transition-all", isAlert ? "border-red-200 ring-1 ring-red-50" : "border-gray-100")}>
            ...
            {loading ? <Skeleton className="h-8 w-10 bg-gray-50 mt-1" /> : <p className={cn("text-3xl font-black text-gray-900", isAlert && "text-red-600")}>{value}</p>}
            <div className="absolute bottom-2 right-2 opacity-10"><Icon size={32} className="text-[#E33636]" /></div>
        </div>
    )
}

function NotifItem({ label, count, icon: Icon, path, color = "text-gray-900", onClick }: { label: string, count: number, icon: any, path: string, color?: string, onClick: () => void }) {
    const router = useRouter();
    return (
        <button onClick={() => { router.push(path); onClick(); }} className="w-full px-5 py-3 hover:bg-gray-50 flex items-center justify-between group transition-colors border-b border-gray-50 last:border-0">
            <div className="flex items-center gap-3">
                <div className="size-8 bg-gray-50 rounded-lg flex items-center justify-center text-[#E33636] group-hover:bg-red-50"><Icon size={16} /></div>
                <p className={cn("text-[10px] font-bold uppercase tracking-tight", color)}>{label}</p>
            </div>
            <div className="flex items-center gap-2"><span className="text-[10px] font-black text-gray-400 bg-gray-100 px-2 py-0.5 rounded-md">{count}</span><ChevronRight size={12} className="text-gray-300" /></div>
        </button>
    );
}