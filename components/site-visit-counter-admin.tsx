...
import { Loader2, RefreshCw, Settings, AlertTriangle, CheckCircle, Keyboard, ChevronDown, FlaskConical } from "lucide-react";
...
export function SiteVisitCounterAdmin() {
  ...
  const fetchStatus = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/site-visit-counter?action=status", {
        headers: {
  ...
  const handleAdjust = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/admin/site-visit-counter", {
        method: "POST",
        headers: {
  ...
  const handleInitialize = async () => {
    ...
    if (!confirm(`Initialize counter with starting number ${num}?\nNext site visit will be PS${new Date().getFullYear()}-${String(num + 1).padStart(3, "0")}`)) {
      return;
    }
